...
export default async function ProductDetailsMenu({
  ...
  params: Promise<{ category: string; id: string; name: string }>;
}) {
  const { category, id, name } = await params;
  ...
  try {
    ...
    if (res.name !== decodeURI(name)) {
      redirect(`/menu/${category}/${id}`, RedirectType.replace);
    }
    response = res;
  } catch (error) {
    redirect(`/menu`, RedirectType.replace);
  }
  const name_category = decodeURI(category);
    ...
    <Layout variant="website">
      <ShowNestedRoute
        list_route={[
          ...
          { path: "/menu", name: "منو" },
          {
            path: `/menu/${decodeURI(category)}`,
            name: `${decodeURI(category)}`,
          },
          {
            path: `/menu/${decodeURI(category)}/${id}`,
            name: `${response.name}`,
          },
        ]}
      />
